...
class Arena{
	...
	public function startTimer(Game $game){
		$this->resetTimer();
		$this->setState(self::STARTING);
		self::$tasks['ticker'] = Base::getAPI()->getScheduler()->scheduleRepeatingTask(new StartTickerTask($game, $this), 20);
	}
	...
	public function resetTimer(){
		$this->setState(self::WAITING);
		if (isset(self::$tasks['ticker'])) Base::getAPI()->getScheduler()->cancelTask(self::$tasks['ticker']->getTaskId());
	...
	public function setState(int $state){
		$this->state = $state;
		Basea::getAPI()->getPluginManager()->callEvent(($ev = new UpdateSignsEvent($this->getOwningGame(), [$this->getOwningGame()->getServer()->getDefaultLevel()], $this)));
		$ev->updateSigns();
	}
