...
class API{
	...
	public static function resetArena(Arena $arena){
		...
		if ($server->isLevelLoaded($levelname)){
			...
			Base::getAPI()->getScheduler()->scheduleAsyncTask(new ArenaAsyncCopyTask($path1, $server->getDataPath(), $levelname));

			// Delayed loading
			Base::getAPI()->getScheduler()->scheduleDelayedRepeatingTask(new class($arena->getOwningGame(), $arena) extends Task{
      ...
      private $plugin;
				...
				public function onRun(int $currentTick){
					if ($this->arena instanceof Arena){
						if (Server::getInstance()->loadLevel($this->levelname)){
							...
							Base::getAPI()->getScheduler()->cancelTask($this->getTaskId());
						}
						$this->tries++;
						if ($this->tries >= 10){
			...
			Base::getAPI()->getScheduler()->scheduleDelayedTask(new class($arena->getOwningGame(), $arena) extends Task{
				public $arena;
